...
int choice;
int ammount;
char phoneNumber[14]; // used char for phoneNumber because we want to include
                      // zero(0) at coutry code eg. +256
int mobileMoneyInit = 1000000; // 1000000; //Mobile money value set to 1Million
int mobileMoney;
int balance;
int saveData();
int data();
...
int saveData() {
  ...
  while ((balance = getw(fptr)) != EOF) {
    ...
    if (ammount <= mobileMoney) {
      printf("You have sent %d shs to %s and your balance is %d", ammount,
             phoneNumber, balance);
      putw(balance, fptr);
...
int airtimeBundles() { printf("airtime bundles"); }
int momoPay() { printf("momo pay"); }
int payements() { printf("payements"); }
int savingsLoans() { printf("Savings"); }
int financialServices() { printf("Hello"); }
int withdrawMoney() { printf("Withdraw money"); }
int myAccount() { printf("My Account"); }
